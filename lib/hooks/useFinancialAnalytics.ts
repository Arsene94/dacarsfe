...
export type FinancialAnalyticsFilters = AnalyticsFilters;

export const useFinancialAnalytics = (
    filters: FinancialAnalyticsFilters = DEFAULT_ANALYTICS_FILTERS,
): UseFinancialAnalyticsResult => {
    const activeFilters = filters ?? DEFAULT_ANALYTICS_FILTERS;

    const summaryKey = useMemo(
        () => buildAnalyticsUrl(API_BASE_URL+ '/analytics/financial/summary', activeFilters),
        [activeFilters],
    );

    const categoriesKey = useMemo(
        () => buildAnalyticsUrl(API_BASE_URL+ '/analytics/financial/categories', activeFilters),
        [activeFilters],
    );
    ...
    } = useSWR<FinancialSummaryApi & Record<string, unknown>>(summaryKey, jsonFetcher, {
    ...
    }, [mutateSummary, mutateCategories, summaryKey, categoriesKey]);

    const isLoading =
        Boolean(summaryKey && typeof summaryResponse === 'undefined' && !summaryError) ||
        Boolean(categoriesKey && typeof categoriesResponse === 'undefined' && !categoriesError);

    const isRefreshing = Boolean(summaryKey && summaryValidating) || Boolean(categoriesKey && categoriesValidating);
    const isError = Boolean((summaryKey && summaryError) || (categoriesKey && categoriesError));
