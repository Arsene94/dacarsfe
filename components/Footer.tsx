import React from 'react';
import { Car, Phone, Mail, MapPin, Clock } from 'lucide-react';
import Link from "next/link";

const Footer = () => {
  return (
    <footer className="bg-berkeley text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {/* Logo și descriere */}
          <div className="space-y-4">
            <div className="flex items-center space-x-2">
              <div className="bg-jade p-2 rounded-lg">
                <Car className="h-6 w-6 text-white" />
              </div>
              <span className="text-2xl font-sans font-bold">DaCars</span>
            </div>
            <p className="text-gray-300 font-sans leading-relaxed">
              Mașini oneste pentru români onești. Predare în aeroport în sub 5 minute, fără taxe ascunse.
            </p>
          </div>

          {/* Linkuri rapide */}
          <div>
            <h3 className="text-lg font-sans font-semibold mb-4">Linkuri Rapide</h3>
            <ul className="space-y-2 font-sans">
              <li>
                <Link href="/" className="text-gray-300 hover:text-jade transition-colors duration-300" aria-label="Acasă">
                  Acasă
                </Link>
              </li>
              <li>
                <Link
                  href="/cars"
                  className="text-gray-300 hover:text-jade transition-colors duration-300"
                  aria-label="Flota Auto"
                >
                  Flota Auto
                </Link>
              </li>
              <li>
                <Link href="#" className="text-gray-300 hover:text-jade transition-colors duration-300" aria-label="Oferte Speciale">
                  Oferte Speciale
                </Link>
              </li>
              <li>
                <Link href="/checkout" className="text-gray-300 hover:text-jade transition-colors duration-300" aria-label="Rezervare">
                  Rezervare
                </Link>
              </li>
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h3 className="text-lg font-sans font-semibold mb-4">Contact</h3>
            <ul className="space-y-3 font-sans text-gray-300">
              <li className="flex items-center space-x-3">
                <Phone className="h-4 w-4 text-jade" />
                <span>+40 723 817 551</span>
              </li>
              <li className="flex items-center space-x-3">
                <Mail className="h-4 w-4 text-jade" />
                <span>contact@dacars.ro</span>
              </li>
              <li className="flex items-center space-x-3">
                <MapPin className="h-4 w-4 text-jade" />
                <span>Aeroportul Henri Coandă, Otopeni</span>
              </li>
              <li className="flex items-center space-x-3">
                <Clock className="h-4 w-4 text-jade" />
                <span>Disponibil 24/7</span>
              </li>
            </ul>
          </div>

          {/* Program și social */}
          <div>
            <h3 className="text-lg font-sans font-semibold mb-4">Program</h3>
            <div className="space-y-2 font-sans text-gray-300">
              <p>Luni - Duminică</p>
              <p className="text-white font-semibold">24/7</p>
              <p className="text-sm mt-4">Predare și ridicare non-stop la aeroport</p>
            </div>
          </div>
        </div>

        <div className="border-t border-gray-600 mt-8 pt-8 flex flex-col md:flex-row justify-between items-center">
          <p className="text-gray-300 font-sans text-sm">
            © 2024 DaCars. Toate drepturile rezervate.
          </p>
          <div className="flex space-x-6 mt-4 md:mt-0">
            <Link
              href="#"
              className="text-gray-300 hover:text-jade transition-colors duration-300 text-sm font-sans"
              aria-label="Termeni și Condiții"
            >
              Termeni și Condiții
            </Link>
            <Link
              href="#"
              className="text-gray-300 hover:text-jade transition-colors duration-300 text-sm font-sans"
              aria-label="Politica de Confidențialitate"
            >
              Politica de Confidențialitate
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
